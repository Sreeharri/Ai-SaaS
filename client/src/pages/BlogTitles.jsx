...
const BlogTitles = () => {
  ...
  const blogCategories =
    [
      'General', 'Technology', 'Health', 'Lifestyle', 'Travel', 'Education', 'Finance', 'Entertainment', 'Food', 'Sports'
    ]
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      setLoading(true);
      const prompt = `Generate a catchy title for a blog post about ${input} in the category of ${selectedCategory}.`;
  ...
  return (
    <div className='h-full overflow-y-scroll p-6 flex items-start flex-wrap gap-4' >
      ...
      <form onSubmit={onSubmitHandler} className='w-full max-w-lg p-4 bg-white rounded-lg border border-gray-200'>
        ...
        <input onChange={(e) => setInput(e.target.value)} value={input} type="text" className='w-full p-2 px-3 mt-2 outline-none text-sm rounded-md border-gray-500' placeholder='The future of AI is....' required />
